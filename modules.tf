...
module "appcd_3e02d187-d1d0-454d-a826-ddab916b4aaf" {
  ...
  custom_role_name = "role_for_spring_petclinic_azure_spring_petclinic_init_kms"
  project_id = local.project_id
  permissions = [
...
module "appcd_436edaac-1b5f-4e57-b729-37eaafb8e74e" {
  source               = "./modules/gcp_role_creation"
  custom_role_name                 = "role_for_spring_petclinic_azure_spring_petclinic_init_cloud_storage"
  project_id = local.project_id
  permissions          = [
    "storage.buckets.get",
    "storage.objects.*",
